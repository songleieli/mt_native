...
- (id)initWithURL:(NSURL *)newURL
{
    ...
    [self setCancelledSelector:@selector(requestCancelled:)];
...
- (void)dealloc
{
    NSLog(@"Finalize Request:%@",self.url);
...
//    NSLog(@"Running Request Count: %i", [sharedQueue operationCount]);
}
...
- (void)reportCancelled
{
    if (delegate && [delegate respondsToSelector:cancelledSelector]) {
        [delegate performSelector:cancelledSelector withObject:self];
    }
    ...
    if (queue && [queue respondsToSelector:@selector(requestCancelled:)]) {
        [queue performSelector:@selector(requestCancelled:) withObject:self];
    }
}
...
- (id)copyWithZone:(NSZone *)zone
{
    ...
    [newRequest setCancelledSelector:[self cancelledSelector]];
...
- (void)setCancelBlock:(ASIBasicBlock)aCancelBlock
{
    [cancelBlock release];
