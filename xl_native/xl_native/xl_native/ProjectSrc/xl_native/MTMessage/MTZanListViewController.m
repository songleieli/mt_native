...
@interface MTZanListViewController ()<ZanListDelegate>
...
-(void)setupUI{
    ...
    self.mainTableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    [self.mainTableView registerClass:ZanListCell.class forCellReuseIdentifier:[ZanListCell cellId]];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(self.mainDataArr.count > 0){
        ZanListCell *cell = [tableView dequeueReusableCellWithIdentifier:[ZanListCell cellId] forIndexPath:indexPath];
        GetLikeMeListModel *model = [self.mainDataArr objectAtIndex:[indexPath row]];
        cell.zanListDelegate = self;
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    return ZanListCellHeight;
}

-(void)btnCellClick:(GetLikeMeListModel*)model{
    
    UserInfoViewController *userInfoViewController = [[UserInfoViewController alloc] init];
    userInfoViewController.userNoodleId = model.videoCommentId;
