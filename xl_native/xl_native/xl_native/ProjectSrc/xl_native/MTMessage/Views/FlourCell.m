...
static NSString* const ViewTableViewCellId = @"FlourCellId";


@implementation FlourCell
...
- (void)fillDataWithModel:(GetFloursModel *)model{
    ...
    [self.imageVeiwIcon sd_setImageWithURL:[NSURL URLWithString:model.flourHead] placeholderImage:[UIImage imageNamed:@"img_find_default"]];
...
- (void)btnDelClick:(id)sender{
    ...
    if ([self.followsDelegate respondsToSelector:@selector(btnCellClick:)]) {
        [self.followsDelegate btnCellClick:self.listModel];
    } else {
        NSLog(@"代理没响应，快开看看吧");
    }
}
