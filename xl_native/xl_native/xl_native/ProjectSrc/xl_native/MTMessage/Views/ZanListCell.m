...
static NSString* const ViewTableViewCellId = @"FZanListCellId";
...
- (void)setup {
    ...
    self.viewBg.size = [UIView getSize_width:ScreenWidth height:ZanListCellHeight];
    ...
    self.imageVeiwIcon.size = [UIView getSize_width:ZanListCellHeight/3*2 height:ZanListCellHeight/3*2];
...
- (void)fillDataWithModel:(GetLikeMeListModel *)model{
    ...
    self.labelTitle.text = model.nickname;
    self.labelSign.text = model.nickname.length == 0?@"暂时还没有签名":model.nickname;
...
- (void)btnDelClick:(id)sender{
    ...
    if ([self.zanListDelegate respondsToSelector:@selector(btnCellClick:)]) {
        [self.zanListDelegate btnCellClick:self.listModel];
    } else {
        NSLog(@"代理没响应，快开看看吧");
    }
}
