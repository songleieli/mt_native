...
#import "CommentMeListCell.h"

static NSString* const ViewTableViewCellId = @"CommentMeListCellId";


@implementation CommentMeListCell
...
- (void)setup {
    ...
    self.viewBg.size = [UIView getSize_width:ScreenWidth height:CommentMeListCellHeight];
    ...
    self.imageVeiwIcon.size = [UIView getSize_width:CommentMeListCellHeight/3*2 height:CommentMeListCellHeight/3*2];
...
- (void)fillDataWithModel:(GetCommentMeListModel *)model{
    
    self.listModel = model;
    [self.imageVeiwIcon sd_setImageWithURL:[NSURL URLWithString:model.commentHead] placeholderImage:[UIImage imageNamed:@"img_find_default"]];
    
    self.labelTitle.text = model.commentNickname;
    self.labelSign.text = model.commentContent.length == 0?@"暂时还没有签名":model.commentContent;
    NSDate *date = [GlobalFunc getDateWithTimeStr:model.commentTime];
...
- (void)btnDelClick:(id)sender{
    ...
    if ([self.commentMeListDelegate respondsToSelector:@selector(btnCellClick:)]) {
        [self.commentMeListDelegate btnCellClick:self.listModel];
    } else {
        NSLog(@"代理没响应，快开看看吧");
    }
}
