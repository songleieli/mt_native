...
#import "AMeListCell.h"

static NSString* const ViewTableViewCellId = @"AMeListCellId";


@implementation AMeListCell
...
- (void)fillDataWithModel:(AMeListModel *)model{
...
- (void)btnDelClick:(id)sender{
    ...
    if ([self.aMeListDelegate respondsToSelector:@selector(btnCellClick:)]) {
        [self.aMeListDelegate btnCellClick:self.listModel];
    } else {
        NSLog(@"代理没响应，快开看看吧");
    }
}
