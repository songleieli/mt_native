...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        _outerView.backgroundColor = [GlobalFunc transformWithHexString:@"#d9d4d1"];
        ...
        _progressLayer.strokeColor = [GlobalFunc transformWithHexString:@"#ff4e00"].CGColor;
        [self setProgress];
...
- (void)longPress:(UIGestureRecognizer *)longPress {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(smartVideoControl:gestureRecognizer:)]) {
        if(self.enableVideoRecord){ //启用longpress 录像功能
