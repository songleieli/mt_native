...
{
    NSDictionary* params = @{@"userid": @"Songleilei", @"timestamp":@([[NSDate date] timeIntervalSince1970] * 1000), @"expires":@(_loginParam.expires)};

    [TCUtil asyncSendHttpRequest:@"get_cos_sign" token:@"15220c88fb29adbb419e4a5c811d7987" params:params handler:^(int resultCode, NSString *message, NSDictionary *resultDict) {
...
//    NSDictionary* params = @{@"userid": _loginParam.identifier, @"timestamp":@([[NSDate date] timeIntervalSince1970] * 1000), @"expires":@(_loginParam.expires)};
    ...
    //Modify by songleilei
    NSDictionary* params = @{@"userid": @"Songleilei", @"timestamp":@([[NSDate date] timeIntervalSince1970] * 1000), @"expires":@(86400)};
    
    
    
    [TCUtil asyncSendHttpRequest:@"get_vod_sign" token:@"15220c88fb29adbb419e4a5c811d7987" params:params handler:^(int resultCode, NSString *message, NSDictionary *resultDict) {
        completion(resultCode, message, resultDict);
    }];
}
...
{
    NSDictionary* hparams = @{@"userid": @"Songleilei", @"timestamp":@([[NSDate date] timeIntervalSince1970] * 1000), @"expires":@(_loginParam.expires)};

    NSMutableDictionary* mparams = [NSMutableDictionary dictionaryWithDictionary:hparams];
    [mparams addEntriesFromDictionary:params];

    [TCUtil asyncSendHttpRequest:@"upload_ugc" token:@"15220c88fb29adbb419e4a5c811d7987" params:mparams handler:^(int resultCode, NSString *message, NSDictionary *resultDict) {
        completion(resultCode, message, resultDict);
    }];
}
