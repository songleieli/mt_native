...
//    [TCUtil report:xiaoshipin_startrecord userName:nil code:result msg:result == 0 ? @"启动录制成功" : @"启动录制失败"];
    if(0 != result){// 0 ? @"启动录制成功" : @"启动录制失败"
        
        if(-3 == result) [self alert:@"启动失败" msg:@"请检查摄像头权限是否打开"];
        if(-4 == result) [self alert:@"启动失败" msg:@"请检查麦克风权限是否打开"];
        if(-5 == result) [self alert:@"启动失败" msg:@"license 验证失败"];
    }else{
        //如果设置了BGM，播放BGM
