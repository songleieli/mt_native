...
{
    UIView *inView = view ? view : [CMPZjLifeMobileAppDelegate shareApp].window;
...
{
    dispatch_async(dispatch_get_main_queue(), ^{
        UIView *inView = [CMPZjLifeMobileAppDelegate shareApp].window;
        MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:inView];
        if (![NSString isEmpty:msg])
        {
...
- (MBProgressHUD *)tipMessage:(NSString *)msg delay:(CGFloat)seconds completion:(void (^)(void))completion
{
    ...
    MBProgressHUD *hud = [MBProgressHUD HUDForView:[CMPZjLifeMobileAppDelegate shareApp].window];
    if (hud == nil) {
        hud = [[MBProgressHUD alloc] initWithView:[CMPZjLifeMobileAppDelegate shareApp].window];
        hud.removeFromSuperViewOnHide = YES;
    }
    [[CMPZjLifeMobileAppDelegate shareApp].window addSubview:hud];
    hud.mode = MBProgressHUDModeText;
    hud.label.text = msg;
    [hud showAnimated:YES];
    [hud hideAnimated:YES afterDelay:seconds];
