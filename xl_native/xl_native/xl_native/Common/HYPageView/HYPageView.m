...
#define TOPBOTTOMLINEBOTTOM_HEIGHT 0.5
...
- (void)setTopTabScrollViewBgColor:(UIColor *)topTabScrollViewBgColor{
    _topTabScrollViewBgColor = topTabScrollViewBgColor;
...
- (instancetype)initWithFrame:(CGRect)frame withTitles:(NSArray *)titles withViewControllers:(NSArray *)controllers withParameters:(NSArray *)parameters withParametersDic:(NSArray*)withParametersDic{
    ...
    _parametersDic = withParametersDic;
    
    return [self initWithFrame:frame withTitles:titles withViewControllers:controllers withParameters:parameters];
...
- (instancetype)initWithFrame:(CGRect)frame withTitles:(NSArray *)titles withViewControllers:(NSArray *)controllers withParametersDic:(NSArray*)withParametersDic{
    ...
    return [self initWithFrame:frame withTitles:titles withViewControllers:controllers withParameters:nil];
...
- (void)layoutSubviews{
    ...
    self.topTabScrollView.backgroundColor = self.topTabScrollViewBgColor;
    
    if ([self.delegate respondsToSelector:@selector(pageView:didSelectIndex:)]){
        [self.delegate pageView:self didSelectIndex:self.currentPage];
    }
}
...
- (UIView *)topTabView{
    if (!_topTabView){
        ...
        if (self.rightButton) {
            self.rightButton.center = CGPointMake(_selfFrame.size.width - self.rightButton.bounds.size.width / 2, TAB_HEIGHT / 2 + __topSpace);
            [_topTabView addSubview:self.rightButton];
        }
        UIView *topTabBottomLine = [UIView new];
        topTabBottomLine.frame = CGRectMake(0, TAB_HEIGHT + __topSpace - TOPBOTTOMLINEBOTTOM_HEIGHT, _selfFrame.size.width, TOPBOTTOMLINEBOTTOM_HEIGHT);
        topTabBottomLine.backgroundColor = self.topTabBottomLineColor; //modify by songlei
        
//        topTabBottomLine.backgroundColor = [UIColor greenColor];
//        topTabBottomLine.height = 2;
...
- (UIScrollView *)topTabScrollView{
    if (!_topTabScrollView){
        ...
        for (NSInteger i=0; i<_titles.count; i++) {
            ...
            equalX += __minSpace + titleSize.width;
        }
        
        CGFloat dividend = _titles.count>1?_titles.count-1:1;
        CGFloat minWidth = (_topTabScrollViewWidth - totalWidth - __leftSpace - __rightSpace) / dividend;
        CGFloat averageX = __leftSpace;
        if (_isAverage) {
        ...
        totalWidth += (_titles.count - 1) * __minSpace + __leftSpace + __rightSpace;
    ...
    if ([keyPath isEqualToString:@"currentPage"]) {
        ...
        for (NSInteger i=0; i<_viewControllers.count; i++) {
            if (page == i) {
                ...
                /*
                 * modify by songlei add _parametersDic
                 */
                if(_parametersDic){//viewController 有多个参数需要传递
                    
                    NSLog(@"------------------");
                    
                    NSDictionary *parameterDic = [_parametersDic objectAtIndex:i];
                    if(parameterDic){
                        NSArray *keys = parameterDic.allKeys;
                        for(NSString *key in keys){
                            if([self getVariableWithClass:viewController.class varName:@"parameter"]){
                                [viewController setValue:[parameterDic objectForKey:key] forKey:key];
                            }
                        }
                    }
                }
                else{//viewController 只有一个参数parameter 需要传递
