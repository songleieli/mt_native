                          ...
                          completion:(void (^)())completion
{
    ...
    for (int i = 0; i < totalIter; i++) {
        ...
        [trs enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            [[transforms objectAtIndex:idx] addObject:obj];
        }];
    }
    ...
    [slices enumerateObjectsUsingBlock:^(CALayer *layer, NSUInteger idx, BOOL *stop) {
        ...
        anim.values = [transforms objectAtIndex:idx];
...
- (NSArray *) sliceImage: (UIImage *) image toLayersAlongAxis: (BCAxis) axis
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        CGImageRef cgImgRef = CGImageCreateWithImageInRect(image.CGImage, rect);
        UIImage *sliceImage = [UIImage imageWithCGImage: cgImgRef
                                                  scale:image.scale
